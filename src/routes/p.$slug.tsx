...
export function ProductDetailPage() {
  ...
  if (notFound) return <><Header /><PageError status={404} message="That product does not exist." /></>
      ...
      <main className="max-w-7xl mx-auto px-6 lg:px-10 py-8">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
                  ...
                  <div className="flex items-center gap-3 flex-wrap mt-4">
                    ...
                    {product.license && (
                      <span
                        ...
                        title={product.license === 'open-source' ? 'Open source' : 'Commercial'}
                      >
                        {product.license === 'open-source' ? (
                          ...
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden><rect width="18" height="11" x="3" y="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg>
                        )}
                        {product.license === 'open-source' ? 'Open source' : 'Commercial'}
                      </span>
                    )}
          ...
          <aside className="lg:w-80 flex-shrink-0 space-y-5">
            ...
            {/* Get the app — only when mobile platform exists */}
            {(product.ios_url || product.android_url) && (
              <div className="bg-surface rounded-card p-6" style={{ boxShadow: cardShadow }}>
                <p className="text-xs text-foreground-faint uppercase tracking-wider font-semibold mb-3">Get the app</p>
                ...
                <p className="text-[11px] text-foreground-faint mt-3 leading-relaxed">
                  Available for {product.platform_codes?.join(', ').replace('mobile', 'iOS + Android')}.
                </p>
              </div>
            )}

            {/* Open source — view source */}
            {product.license === 'open-source' && product.source_url && (
              <div className="bg-surface rounded-card p-6" style={{ boxShadow: cardShadow }}>
                <p className="text-xs text-foreground-faint uppercase tracking-wider font-semibold mb-3">Open source</p>
                <a
                ...
                <p className="text-[11px] text-foreground-faint mt-3 leading-relaxed">
                  Source code is publicly available. Star, fork, or contribute.
                </p>
              </div>
            )}
